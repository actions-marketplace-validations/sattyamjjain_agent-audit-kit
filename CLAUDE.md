...
- **316 rules** across 13 security categories
- **93 scanner modules** including AST-based Python taint analysis plus regex dangerous-sink pattern scanners for TypeScript/JavaScript and Rust (pattern matching, not taint flow)
...
agent_audit_kit/
  ...
  rules/
    builtin.py         # 316 RuleDefinition entries (rule registry)
  scanners/            # 93 registered scanners (97 .py files on disk — the registry is authoritative)
...
- **Rule registry**: `rules/builtin.py` defines all 316 rules as `RuleDefinition` dataclasses in a global `RULES` dict, populated by `_r()` helper.
