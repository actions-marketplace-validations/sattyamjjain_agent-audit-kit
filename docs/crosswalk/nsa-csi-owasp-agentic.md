...
Every AgentAuditKit rule (316 total; 303 mapped) against two agentic-security standards. Static and deterministic — generated from the committed rule registry and compliance mappings, no scan required.
...
**Composition** — three rules in this table (`AAK-COMPOSE-001/002/003`) hold over a *graph* of components rather than one artifact, because a chain can cross a control boundary while every component on it satisfies the control individually. Source results: CompoSkill ([arXiv:2608.16246](https://arxiv.org/abs/2608.16246)), which assembles a risk chain from skills that each pass an individual scanner and reports attack success falling off past three skills, and ColluSkill ([arXiv:2608.09732](https://arxiv.org/abs/2608.09732)), whose skills cooperate through a medium neither declares. Read the CSI controls below as per-component requirements; these three are what checks the joins between them.

**Evidence** — for how often these controls actually fail in the wild, see *The State of MCP Security, 2026* (v1.0): a scan of 2,303 distinct public MCP configs. [Report](https://github.com/sattyamjjain/agent-audit-kit/blob/main/research/state-of-mcp-2026/REPORT.md) · this crosswalk says what the controls are; the report says how often they are met.
...
| `AAK-AGENT-ZERO-CVE-2026-30624-PIN-001` | high | supply-chain | Design for boundaries; Validate parameters; Constrain and sandbox tool execution; Instrument for logging and detection | ASI02 Tool Misuse; ASI10 Rogue Agents |
...
| `AAK-COMPOSE-001` | high | composition | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
| `AAK-COMPOSE-002` | high | composition | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
| `AAK-COMPOSE-003` | medium | composition | Design for boundaries; Validate parameters; Constrain and sandbox tool execution; Filter and monitor output pipelines and chained execution | ASI05 Unexpected Code Execution |
...
| `AAK-MCP-APIFY-CVE-2026-46341-001` | high | supply-chain | — | ASI06 Memory & Context Poisoning |
...
| `AAK-MCP-ATLASSIAN-CVE-2026-73498-001` | high | supply-chain | Design for boundaries; Validate parameters; Constrain and sandbox tool execution; Filter and monitor output pipelines and chained execution | ASI05 Unexpected Code Execution |
...
| `AAK-MCP-AUTHFETCH-CVE-2026-49857-001` | high | supply-chain | — | ASI06 Memory & Context Poisoning |
...
| `AAK-MCP-CORTEX-CVE-2026-49986-001` | high | supply-chain | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
...
| `AAK-MCP-N8N-CVE-2026-72768-001` | high | supply-chain | Design for boundaries; Validate parameters; Constrain and sandbox tool execution; Filter and monitor output pipelines and chained execution | ASI06 Memory & Context Poisoning; ASI05 Unexpected Code Execution |
...
| `AAK-POLICY-TRUNCATION-001` | high | trust-boundary | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
...
| `AAK-SHELL-DEFAULT-PROFILE-001` | high | mcp-config | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
| `AAK-SHELL-QUOTED-INTERP-001` | high | taint-analysis | Design for boundaries; Validate parameters; Constrain and sandbox tool execution | ASI02 Tool Misuse |
