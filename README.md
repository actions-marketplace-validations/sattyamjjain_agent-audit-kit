...
- **Remediation guidance: all <!-- rule-count:total -->319<!-- /rule-count --> of <!-- rule-count:total -->319<!-- /rule-count --> rules.** Every finding tells you what to change and why — that is the product, and it has no gap.
- **Mechanical fix recipes: <!-- fix-recipe-coverage:count -->11<!-- /fix-recipe-coverage --> of <!-- rule-count:total -->319<!-- /rule-count --> rules (<!-- fix-recipe-coverage:pct -->3.4<!-- /fix-recipe-coverage -->%).** This is a scope decision, not coverage: **a recipe ships only where the remediation is deterministic and one-line** — exactly one correct edit, confirmable from the diff. Everything else stays advisory on purpose, because a fix that needs judgement is a fix that can be wrong silently. `agent-audit-kit fix` applies these; the narrower `suggest --auto-pr` allow-list is the subset AAK will open a PR for.
...
> **The weekly snapshot is not currently weekly, and the trend chart has never
> been possible.** Two separate faults, both fixed in v0.3.86, neither yet proven
> over consecutive weeks.
>
> The scheduled crawl died on `HTTP 429` — the status the GitHub Search API
> returns for a secondary rate limit, which the retry path did not handle — so
> every Monday run from 2026-06-15 failed except 2026-07-20. It now backs off on
> 429, keeps a partial crawl under a wall-clock budget rather than being killed
> with nothing, records `partial` and its reason in the artifact, and files an
> issue when a scheduled run fails; the absence of that last part is why two
> months passed unnoticed.
>
> Separately, and independently of the failures: the site build removed its
> output directory before reading the previous `history.json`, so every run
> appended one entry to an empty list. **Every published history in this
> project's life has held exactly one snapshot**, which is why the page says
> "not enough snapshots yet for a trend chart (need >= 2)" even after a run
> succeeds. The prior history is now seeded into the build. The trend chart
> should appear from the second successful run onward, and this note comes down
> when it does.
...
cd agent-audit-kit
pip install -e ".[dev]"
pytest -v                          # <!-- test-count:total -->1,873<!-- /test-count --> test functions
ruff check .                       # Lint
