...
> **The weekly snapshot is not currently weekly.** The scheduled crawl asked for
> more of the GitHub Search budget than its token allows, and a rate-limit wall
> was fatal rather than partial, so every Monday run from 2026-06-15 failed
> except 2026-07-20. `history.json` therefore holds **one** snapshot, from that
> date, and the site says so rather than drawing a trend line through a single
> point. v0.3.86 sizes the crawl to the budget, keeps a partial crawl instead of
> losing the week, and files an issue when a scheduled run fails — the absence of
> which is why this went unnoticed for two months. The claim here will be
> restored to "weekly" once consecutive Monday runs have actually landed.
...
cd agent-audit-kit
pip install -e ".[dev]"
pytest -v                          # <!-- test-count:total -->1,872<!-- /test-count --> test functions
ruff check .                       # Lint
