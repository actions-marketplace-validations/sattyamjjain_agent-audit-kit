...
<p align="center">
  ...
  <!-- fp-badge --><a href="benchmarks/false_positive/RESULTS.md"><img src="https://img.shields.io/badge/benign--slice%20536%20configs-HIGH%2FCRIT%20FP%202%2F4-orange.svg" alt="Benign-slice false-positive measurement: 536 configs scanned, 2 of 4 HIGH/CRITICAL findings were false positives (50.0%)"></a><!-- /fp-badge -->
  ...
  <a href="#what-it-scans"><img src="https://img.shields.io/badge/rules-327-blue.svg" alt="Rules: 327"></a>
...
- **<!-- rule-count:total -->327<!-- /rule-count --> rules** across 14 security categories, covering the 2026 CVE wave
  - Rule count is computed from the registry and verified in CI (`test_rule_count_is_canonical`).
- **<!-- scanner-count:total -->95<!-- /scanner-count --> scanner modules** including AST-based Python taint analysis and regex pattern scanners for TypeScript/JavaScript and Rust
- **Remediation guidance: all <!-- rule-count:total -->327<!-- /rule-count --> of <!-- rule-count:total -->327<!-- /rule-count --> rules.** Every finding tells you what to change and why — that is the product, and it has no gap.
- **Mechanical fix recipes: <!-- fix-recipe-coverage:count -->11<!-- /fix-recipe-coverage --> of <!-- rule-count:total -->327<!-- /rule-count --> rules (<!-- fix-recipe-coverage:pct -->3.4<!-- /fix-recipe-coverage -->%).** This is a scope decision, not coverage: **a recipe ships only where the remediation is deterministic and one-line** — exactly one correct edit, confirmable from the diff. Everything else stays advisory on purpose, because a fix that needs judgement is a fix that can be wrong silently. `agent-audit-kit fix` applies these; the narrower `suggest --auto-pr` allow-list is the subset AAK will open a PR for.
...
jobs:
  scan:
    ...
    steps:
      ...
      - uses: sattyamjjain/agent-audit-kit@v0.3.91
        id: scan
        with:
          fail-on: high
...
| **Transport Security** | <!-- category-count:TRANSPORT_SECURITY -->14<!-- /category-count --> | HTTP endpoints, TLS disabled, deprecated SSE, tokens in URL query strings, transport body-size limits (CVE-2026-39313), SSRF redirect bypass (CVE-2026-41481), DNS-rebinding (CVE-2025-66414/66416, CVE-2026-35568/35577) |
...
**<!-- rule-count:total -->327<!-- /rule-count --> rules total.** Every finding includes severity, evidence, remediation, OWASP references, Adversa references, and CVE links where applicable.
...
| `--sessions PATH` | | Run the session-scoped rules over agent transcripts — OpenAI Agents SDK traces, LangGraph checkpoints, JSONL. See [Session transcripts](docs/session-transcripts.md) |
...
| Detection rules (static) | **<!-- rule-count:total -->327<!-- /rule-count -->** | Runtime policies, not rules | ~30 | LLM-assisted |
...
| A2A protocol scanning | **<!-- category-count:A2A_PROTOCOL -->13<!-- /category-count --> rules** | Agent Mesh | No | No |
...
deterministically (scan date **2026-07-26**). Headline: **<!-- report:noauth-pct -->52.2<!-- /report -->% (<!-- report:noauth-n -->1,203<!-- /report -->/2,303) declare
...
**Citing it.** The report is **version 1.0** (published 2026-08-17, scan date
2026-07-26) and has a stable identity so a number in it can be referenced:
[how to cite](research/state-of-mcp-2026/REPORT.md#how-to-cite-this-report) gives a
plain-text and a BibTeX form, and [`CITATION.cff`](CITATION.cff) carries the same
identity for GitHub's "Cite this repository" widget. The
[NSA-CSI / OWASP-Agentic crosswalk](docs/crosswalk/nsa-csi-owasp-agentic.md) links to
it too: the crosswalk says what the controls are, the report says how often they are
met.
...
cd agent-audit-kit
pip install -e ".[dev]"
pytest -v                          # <!-- test-count:total -->2,001<!-- /test-count --> test functions
ruff check .                       # Lint
